...
const SimilarPostsPage = () => {
  ...
  const { id, sort, orderBy } = router.query
  ...
  const originPost = useMemo<IPost | undefined>(
    () => data?.pages[0]?.result.origin,
    [data]
  )
  const postId = useMemo(() => originPost?.postId, [originPost])
  ...
  return (
    <div className='w-full max-w-[100rem] px-2 md:px-28 mb-8 flex flex-col items-stretch'>
      <Head>
        <title>Similar Posts - Here News</title>
      </Head>

      <div className='flex flex-row flex-wrap py-2 my-4 justify-between items-center gap-2 min-w-['>
        <Typography
          type='h2'
          className='font-semibold !text-xl !leading-7 flex-1 text-header line-clamp-1'
        >
          {postTitle}
        </Typography>
          ...
          <Popover className='mt-6 right-0'>
            ...
            {() => (
              <div className='flex flex-col w-[7.875rem] h-32'>
                <Link
                  href={`/similarPosts/${postId}?orderBy=time&sort=latest`}
                ...
                <Link
                  href={`/similarPosts/${postId}?orderBy=time&sort=earliest`}
      ...
      <div className='w-full'>
        ...
        >
          {isSuccess && data.pages.length > 0 ? (
            data?.pages?.map(page =>
              page?.result?.result?.map((post: IPost) => (
